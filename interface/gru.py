...
class GRU(nn.Module):
    ...
    def __init__(self, number_of_features, number_of_classes, number_of_layers, batch_first):
        ...
        self.gru = nn.GRU(number_of_features, number_of_classes, num_layers=number_of_layers, batch_first=batch_first)
...
def take_data_gru(input_path, length):
    data_lines = []
...
def train_gru(X, Y, model, optimizer, loss_function, device, epoch, streaming):
